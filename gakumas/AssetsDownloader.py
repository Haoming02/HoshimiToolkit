...
def downloadAll(jDict: dict, maxThread: int):
    ...
    console.print(
        f"[bold green]>>> [Succeed][/bold green] Start downloading assets, this may take some hours...\n"
    )
    executor = ThreadPoolExecutor(max_workers=maxThread)
    urlFormat = jDict["urlFormat"]
    allTasks = [
        executor.submit(downloadOne, it, "assetbundle", urlFormat)
        for it in assetBundleList
    ]
    allTasks.extend(
        [
            executor.submit(downloadOne, it, "resources", urlFormat)
...
def downloadOne(it: Dict, _type: str, url_format: str):
    ...
    if md5 in fileNames:
        ...
        return
    url = (
        url_format.replace("{v}", str(it.get("uploadVersionId")))
        .replace("{type}", _type)
    ...
    try:
        ...
        errCount += 1
        console.print(
            f"[bold red]>>> ({count}/{countAll}) [Error][/bold red] Failed to download '{md5}'."
        )
        lock.release()
